...
2. 仓库操作：
   ...
   ◦ 关联远程仓库：git branch -M main → git remote add origin https://github.com/2025BIUHcourse-classroom-group4-week1-homework-1-template.git；

◦ 推送作业：git push -u origin main（解决TLS连接异常后完成推送）；

3. 截图收集：！[安装常用工具](image/安装.png) ！[推送作业](image/提交.png)
## 3.遇到的问题与解决方法
### 3.1
...
### 3.2 
问题2：PAT直接输入终端提示“未找到命令”

• 原因：误将GitHub个人访问令牌（PAT）在普通命令行输入，而非密码提示符下粘贴；

• 尝试过程：重新执行git push，在Password for 'https://ChenL777@github.com':提示符下粘贴PAT，验证令牌有效性；

• 解决结论：PAT仅能在密码输入环节使用，不可直接作为终端命令执行。

### 3.3  
问题3：Ubuntu无网络+触控板无法使用

• 原因：网卡驱动未加载,ubuntu内核过低，需要升级内核；

• 尝试过程：
1. 网络问题：通过手机USB共享网络临时解决；
2. 触控板问题：内核过底，升级内核，解决触控板不能使用问题；
•
 解决结论：优先通过手机共享网络恢复网络环境，再逐步修复硬件驱动问题，触控板故障可临时用外接鼠标替代。
...
3. 排障层面：学会从终端报错信息定位问题根源，针对路径、网络、认证类错误，能通过指令调整、方式替换解决问题；
